...
class AboutTrueAndFalse < Neo::Koan
  ...
  def test_true_is_treated_as_true
    assert_equal :true_stuff, truth_value(true) #------------> :true_stuff é o retorno da função truth_value
  end

  def test_false_is_treated_as_false
    assert_equal :false_stuff, truth_value(false)#------------> :false_stuff é o retorno da função truth_value
  end

  def test_nil_is_treated_as_false_too
    assert_equal :false_stuff, truth_value(nil)# vazio é false
  end
